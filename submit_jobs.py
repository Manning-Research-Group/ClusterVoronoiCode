...
dt = 0.0001
ncells = int(200)
trial_amount = np.linspace(0,9,10)

## Set the executable
runfile = '/home/elawsonk/cellGPU-master/gapFIRE.out'
...
for trial in trial_amount:
    for p in p0_vals:
        for run in range(num_runs_per_param_set):
            ...
            bashfilename = 'gap_' + str(ncells) + '_' + str(p).replace('.', 'p') + '_'  + str(trial)  + '.sh'
            newsubfilename = 'gap_' + str(ncells) + '_' + str(p).replace('.', 'p') + '_'  + str(trial)  + '.sub'
    ...
    """ + str(runfile) + """ -e """ + str(dt) + """ -n """ + str(ncells) + """ -p """ + str(p) + """ -h """ + str(trial) 
                     )
    ...
    output = gap_""" + str(ncells) + '_'  + str(p) + '_'  + str(trial) + """.out
    error = gap_""" + str(ncells) + '_' + str(p) + '_'  + str(trial)  + """.err
    log = gap_""" + str(ncells) + '_' + str(p) + '_'  + str(trial)  +""".log
