...
void voronoiModelBase::movePointsCPU(GPUArray<Dscalar2> &displacements,Dscalar scale)
    ...
    if(scale == 1.)
        ...
        for (int idx = 0; idx < Ncells; ++idx)
            ...
            //h_p.data[idx].x += -scale*(1.0*0.01);
            //h_p.data[idx].y += scale*0.0;
            Box->putInBoxReal(h_p.data[idx]);
            };
        }
    };
...
void voronoiModelBase::globalTriangulationCGAL(bool verbose)
    ...
    //dcgal.PeriodicTriangulationNineSheeted(Psnew,b1,b2,b3,b4);
...
void voronoiModelBase::compressBox(Dscalar percentChangeX, Dscalar percentChangeY)
    {
     ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host, access_mode::read);
     Dscalar b11,b12,b21,b22;
     Box->getBoxDims(b11,b12,b21,b22);
     // Note: originalBoxsize is a public variable defined in Simple2DCell::setCellPositionsRandomly
     Dscalar newBoxHeight = b22 + sqrt(Ncells)*percentChangeY;
     Dscalar newBoxLength = b11 - sqrt(Ncells)*percentChangeX;

     Box->setGeneral(newBoxLength, b12, b21, newBoxHeight);
     for (int v=0; v<Ncells; ++v)
      {
       Dscalar newx = h_p.data[v].x - sqrt(Ncells)*percentChangeX;
       Dscalar newy = h_p.data[v].y - sqrt(Ncells)*percentChangeY;
       h_p.data[v].x = newx;
       h_p.data[v].y = newy;
      }
    };