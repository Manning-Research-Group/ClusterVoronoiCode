      ...
      */
	  ...
	  for (int vv = 0; vv < 3; ++vv)
		  ...
		  for(int cc =0; cc <3; ++cc)
		      {
		        // cell neighbor cc of vertex1 and cell neighbor cc of vertex 2
		        cell1 = h_vcn.data[3*vertex1+cc];
		        cell2 = h_vcn.data[3*vertex2+cc];  
		        // if one of the cells are type 1, set typecell to 1   
		        if((h_ct.data[cell1] == 1)||(h_ct.data[cell2] == 1)){typecell=1;}
		  ...
		  if(norm(edge) < T1Threshold)
                    {
		      //Gaps shouldn't T1 and merging event
		      if((h_ct.data[cellSet.y] == 1)&&(h_ct.data[cellSet.w] ==1))
			{  
			...
			}; //end else condition on type
		      
		    };//end condition on length
		};
...
/*!
Trigger a gap formation event. This will take a vertex and form a gap cell
*/
void vertexModelBase::gapform(int vertIndex)
    {
    //We will replicate some of what is done in cell death. We want to locate the cells around this vertex and the verticies connected to it.
    vector<int> cells(3);
    ...
    vector<int> newcindx(9); // Has the indicies of the cells surounding the new verticies
    vector<int> newvindx(12); // Has indicies of the cell verticies newvindx[4*cc] represents the verticies of the gap those inbetween represent the verticies cc neighborings the initial
    vector<int> newcell(3); newcell[0]=vertIndex; newcell[1]=Nvertices; newcell[2]=Nvertices+1; //The verticies of the new cell 
    vector<int> connectingv(3);
    vector<int> c0Vertices; 
    vector<int> c1Vertices;
    vector<int> c2Vertices;
    ...
    Dscalar2 newVertexPosition,v0pos,v1pos,v2pos;
    Dscalar2 oldVertexPosition=h_v.data[vertIndex];
    Dscalar2 cellPos;
    Dscalar2 deltaPos;
    ...
    for(int cc =0; cc <3; ++cc)
        {
            cells[cc] = h_vcn.data[3*vertIndex+cc];    
            //cout << cells[cc] << endl;   
        }

    //Unsure what to put for these two
    int v1idx=h_vn.data[3*vertIndex];
    int v2idx=h_vn.data[3*vertIndex+1];
    //This is just to run the cell division code by effectively dividing one of the cells. This will just copy parameters to the new cell can be adjusted later.
    vector<int> cdtest(3); cdtest[0]=cells[0]; cdtest[1] = vertIndex; cdtest[2] = vertIndex;

    //Find the position of the newly created verts in this case on the line between the splitting one and the others.
    for(int cc =0; cc <3; ++cc)
            {
            deltaPos=(oldVertexPosition-h_v.data[h_vn.data[3*vertIndex+cc]]);
            //cout << deltaPos.x << endl;  

            if(deltaPos.x>0.9*sqrt(200)){deltaPos.x+=-sqrt(200);}
            if(deltaPos.x<-0.9*sqrt(200)){deltaPos.x+=sqrt(200);}
            if(deltaPos.y>0.9*sqrt(200)){deltaPos.y+=-sqrt(200);}
            if(deltaPos.y<-0.9*sqrt(200)){deltaPos.y+=sqrt(200);}

            //cout << deltaPos.x << endl;            
            newVertexPosition = oldVertexPosition-0.1*deltaPos;
            connectingv[cc] = h_vn.data[3*vertIndex+cc];
            //cout << connectingv[cc] << endl;
            //cout << newcell[cc] << endl;
            for(int vv =0; vv < 3; ++vv)
                {
                    //Find the cells neighboring these verts
                    if(std::count(cells.begin(),cells.end(),h_vcn.data[3*h_vn.data[3*vertIndex+cc]+vv]))
                        {
                        	newcindx[3*cc+vv]=h_vcn.data[3*h_vn.data[3*vertIndex+cc]+vv];
                        }
                    else
                        {newcindx[3*cc+vv]=Nvertices/2;}
                }
            for(int vv=0; vv < 4; ++vv)
                {//Find the vertex neighbors of the new verts
                    if(vv==0){
                        newvindx[4*cc+vv]=newcell[cc];
                    }
                    else if(vv==1){
                       newvindx[4*cc+vv]=h_vn.data[3*vertIndex+cc];
                       //cout << newvindx[4*cc+vv] << endl;
                    }
                    else{
                        newvindx[4*cc+vv]=newcell[(cc+vv-1)%3];
                    }
                    //cout << newvindx[4*cc+vv] << endl;
                }

            int neigh = h_cvn.data[cells[cc]];
            if(cc>0)
                {//Place the new verts
                    //Box->putInBoxReal(newVertexPosition);
                    if(cc==1){v1pos=newVertexPosition;}
                    else{v2pos=newVertexPosition;}
                }
            else
                {
                    v0pos=newVertexPosition;
                }       
            }
    
    //Gives the verticies of all the surrounding cells
    {
    ArrayHandle<int> h_cv(cellVertices);
    ArrayHandle<int> cellVertNum(cellVertexNum);
        ...
        for(int cc=0; cc <3; ++cc)
            {        
                ...
                vector<int> newindex(6,-1);
                int neighs=0; int cellIdx=0; int v0=0; int k=1; int minindx=-1; int maxindx=-1; int indxcheck=-1; int vertadded=0;
                cellIdx = cells[cc];       

                //printf("Cell Number: ");
                //cout << cellIdx << endl;
                neighs = cellVertNum.data[cellIdx];
                combinedVertices.reserve(neighs+1);
                for (int i = 0; i < neighs; ++i)
                    {
                        v0=h_cv.data[n_idx(i,cellIdx)];
                        combinedVertices.push_back(h_cv.data[n_idx(i,cellIdx)]);
                        for(int j = 0; j<3; ++j)
                            {
                                if(v0==vertIndex)
                                    {
                                        newindex[0]=i;
                                        newindex[1]=vertIndex;
                                    }

                                if(v0==connectingv[j])
                                    {
                                        newindex[2*k]=i;
                                        newindex[2*k+1]=newcell[j];
                                        k+=1;           
                                    }
                            }
                    }
                    	...
                    	if(newindex[2*minindx+1]!=vertIndex)
                    	{   
	                        indxcheck=abs(newindex[0]-newindex[2*minindx]);
	                        if(indxcheck!=1){indxcheck=0;}
	                        else{indxcheck=max(newindex[0],newindex[2*minindx]);}
	                        combinedVertices.insert(combinedVertices.begin()+indxcheck,newindex[2*minindx+1]);
                            vertadded+=1;
                    	}

                    	if(newindex[2*maxindx+1]!=vertIndex)
                    	{   
	                        indxcheck=abs(newindex[0]-newindex[2*maxindx]);
	                        if(indxcheck!=1){indxcheck=0;}
	                        else{indxcheck=max(newindex[0],newindex[2*maxindx]);}
	                        combinedVertices.insert(combinedVertices.begin()+indxcheck+vertadded,newindex[2*maxindx+1]);
                    	}

                    	if(newindex[2*maxindx+1]!=vertIndex and newindex[2*minindx+1]!=vertIndex)
                    		{
                    			indxcheck=0;
                    			if(newindex[0]>newindex[2]){indxcheck+=1;}
                    			if(newindex[0]>newindex[4]){indxcheck+=1;}
                    			combinedVertices.erase(combinedVertices.begin()+newindex[0]+indxcheck);
                    		}


                if(cc==0){c0Vertices=combinedVertices;}
                if(cc==1){c1Vertices=combinedVertices;}
                if(cc==2){c2Vertices=combinedVertices;}
            }
    }

    Index2D n_idxOld(vertexMax,Ncells);
    //Add the new cell
    Simple2DActiveCell::cellDivision(cdtest);
    Nvertices += 2;
    ...
    //first, let's take care of the vertex positions, masses, and velocities
        ...
        h_vp.data[vertIndex] = v0pos;
        h_vp.data[Nvertices-2] = v1pos;
        h_vp.data[Nvertices-1] = v2pos;
        ArrayHandle<Dscalar2> h_vvel(vertexVelocities);
        h_vvel.data[Nvertices-2] = make_Dscalar2(0.0,0.0);
        h_vvel.data[Nvertices-1] = make_Dscalar2(0.0,0.0);
   ...
   //the vertex-vertex neighbors
        ...
        //Updated v0
        h_vv.data[3*(vertIndex)+0] = newvindx[1];
        h_vv.data[3*(vertIndex)+1]= newvindx[2];
        h_vv.data[3*(vertIndex)+2]= newvindx[3];   
        //new v1
        h_vv.data[3*(Nvertices-2)+0] = newvindx[5];
        h_vv.data[3*(Nvertices-2)+1] = newvindx[6];
        h_vv.data[3*(Nvertices-2)+2] = newvindx[7];
        //new v2
        h_vv.data[3*(Nvertices-1)+0] = newvindx[9];
        h_vv.data[3*(Nvertices-1)+1] = newvindx[10];
        h_vv.data[3*(Nvertices-1)+2] = newvindx[11];
        //Fix old verts to new
        for (int cc = 0; cc < 3; ++cc)
        {
            if(h_vv.data[3*(newvindx[5])+cc] == vertIndex){h_vv.data[3*(newvindx[5])+cc] =(Nvertices-2);}
            if(h_vv.data[3*(newvindx[9])+cc] == vertIndex){h_vv.data[3*(newvindx[9])+cc] =(Nvertices-1);}
        }

        //Check Vertex
        //int testvertex1=22;
        //cout << h_vv.data[3*(testvertex1)+0] <<endl;
        //cout << h_vv.data[3*(testvertex1)+1] <<endl;
        //cout << h_vv.data[3*(testvertex1)+2] <<endl;
        };

    //Update the neighboring cell lists of the new verts
    {
    	ArrayHandle<int> h_vv(vertexNeighbors);
    	ArrayHandle<int> h_cv(cellVertices);
        ArrayHandle<int> h_vcn(vertexCellNeighbors);
        ArrayHandle<int> cellVertNum(cellVertexNum);
        //Update old v0
        h_vcn.data[3*(vertIndex)+0] = newcindx[0];
        h_vcn.data[3*(vertIndex)+1] = newcindx[1];
        h_vcn.data[3*(vertIndex)+2] = newcindx[2];       
        //new v1
        h_vcn.data[3*(Nvertices-2)+0] = newcindx[3];
        h_vcn.data[3*(Nvertices-2)+1] = newcindx[4];
        h_vcn.data[3*(Nvertices-2)+2] = newcindx[5];
        //new v2
        h_vcn.data[3*(Nvertices-1)+0] = newcindx[6];
        h_vcn.data[3*(Nvertices-1)+1] = newcindx[7];
        h_vcn.data[3*(Nvertices-1)+2] = newcindx[8];

        //cellVertNum.data[cells[0]] +=1;
        //cellVertNum.data[cells[1]] +=1;
        //cellVertNum.data[cells[2]] +=1;

    	//cout << h_cv.data[cells[0]] << endl;
    	//cout << cellVertNum.data[cells[0]] << endl;
    	//cout << cellVertNum.data[cells[1]]  << endl;
    	//cout << cellVertNum.data[cells[2]] << endl;
    };
   ...
   // finally, reset the vertices associated with every cell
        ...
        //correct surrounding cell verts
        for (int cc = 0; cc < 3; ++cc)
        	{
        		if(cc==0){combinedVertices=c0Vertices;}
                if(cc==1){combinedVertices=c1Vertices;}
                if(cc==2){combinedVertices=c2Vertices;}
        		int verts=h_cvn.data[cells[cc]];
        		for (int vv = 0; vv < verts+1; ++vv)
        			{
		            	cv.data[n_idx(vv,cells[cc])] = combinedVertices[vv];
        			}

        	}
        //add the vertices to the new cell
        cv.data[n_idx(0,Ncells-1)] = vertIndex;
        cv.data[n_idx(1,Ncells-1)] = Nvertices-2;
		cv.data[n_idx(2,Ncells-1)] = Nvertices-1;
		
        
        //New cell position
        ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::readwrite);
        h_p.data[Ncells-1]=oldVertexPosition;
   
        //New cell is a different type with new A0 and P0
        ArrayHandle<Dscalar2> h_APP(AreaPeriPreferences,access_location::host,access_mode::readwrite);
        ArrayHandle<int> h_ct(cellType,access_location::host,access_mode::readwrite);
        h_ct.data[Ncells-1] = 1;
        //New Area
        h_APP.data[Ncells-1].x = 0.02*h_APP.data[cells[0]].x;
        //New Perimeter
        h_APP.data[Ncells-1].y = sqrt(0.02)*4.55;
        //Adjust Modulii
        ArrayHandle<Dscalar2> h_m(Moduli,access_location::host,access_mode::overwrite);
        //h_m.data[Ncells-1].x = 100;
        //h_m.data[Ncells-1].y = 1;
        h_m.data[Ncells-1].x = 0.0;
        h_m.data[Ncells-1].y = 0.0;

        for(int cc =0; cc <3; ++cc)
        	{
            	h_cvn.data[cells[cc]]=h_cvn.data[cells[cc]]+1;
        	}
            cellVertNum.data[Ncells-1]=3;
        };

    //printf("End of Gap Form \n");
    //cout << Ncells << endl;
    };
...
void vertexModelBase::cellDeath(int cellIndex)
    ...
    for (int vv = 0; vv < 3; ++vv)
        ...
        //Tells what the verticies are (387,388,363)
        vertices[vv] = vIndex;
        if(vIndex < smallestV)
            {
            //This autohits once as smallestV starts out of range at 401
            smallestV = vIndex;
            smallestVIndex = vv;
            //Loop locates the smallest vertex which is 363 here which is the 3rd index which is index 2)
            };
        for (int cc =0; cc <3; ++cc)
            {
            //I think this locates the cells that shares two? vertice with this dying cell (6,79,142),(79,169,142),(6,142,169)
    ...
    for (int vv = 0; vv < 3; ++vv)
        ...
        for (int v2 = 0; v2 < 3; ++v2)
            ...
            if(testVertex != vertices[0] && testVertex != vertices[1] && testVertex != vertices[2])
                {
                //Finds verticies that neighbors have that aren't in dying cell this seems to include them which feels like a bug (fixed with bracket)
                newVertexNeighbors.push_back(testVertex);
                //cout << testVertex << endl;
                }
            };
        };
    ...
    //Eventually, put the new vertex in, say, the centroid... for now, just put it on top of v0
    ...
    for (int oldCell = 0; oldCell < 3; ++oldCell)
        ...
        //Cells sharing two verts (6,79,169)
        int neigh = h_cvn.data[cIdx];
        //The number of vertices for those cells ~ (9,8,7) which seems like a lot...
        //fun solution: if the cell includes either v1 or v2, replace with v1 and delete duplicates
        vector<int> vNeighs(neigh);
        for (int vv = 0; vv < neigh; ++vv)
            {
            int vIdx = h_cv.data[n_idx(vv,cIdx)];
            //These are each of the verticies of the adjacent cells
        ...
        //cout << h_cvn.data[cIdx] << endl;
        //Number of connected verticies to each dying one
...
void vertexModelBase::cellDivision(const vector<int> &parameters, const vector<Dscalar> &dParams)
    ...
    //cout << v2 << endl;
    cout << neighs << endl;
    //Gives the verticies of the cell that is falling apart 248, 400, 81, 252 ,401, 250, 247, 79
    combinedVertices.reserve(neighs+2);
    for (int i = 0; i < neighs; ++i){
        combinedVertices.push_back(cv.data[n_idx(i,cellIdx)]);
    	//cout << combinedVertices[i] << endl;
    }
    ...
    //cout << combinedVertices[neighs] << endl;

    //for (int i = 0; i < neighs+2; ++i){
	//	cout << combinedVertices[i] << endl;
    //}
    ...
    //Index of verts b4 new verts
    v1idx = cv.data[n_idx(v1,cellIdx)];
    v2idx = cv.data[n_idx(v2,cellIdx)];
    //Locates the index of the new vert 
    if (v1 < neighs - 1)
