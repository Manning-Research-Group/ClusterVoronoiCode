...
void Simulation::setBoxAndScale(BoxPtr _oldbox, BoxPtr _newbox){   
auto cellConf = cellConfiguration.lock();
ArrayHandle<Dscalar2> h_p(cellConf->returnPositions(),access_location::host,access_mode::readwrite);
  for (int ii = 0; ii < cellConf->getNumberOfDegreesOfFreedom(); ++ii)
  {
    Dscalar2 ans;
    Dscalar2 pos = h_p.data[ii];
    _oldbox->invTrans(pos,ans);
    _newbox->Trans(ans,pos);
    h_p.data[ii]=pos;
  }
setBox(_newbox);
}
